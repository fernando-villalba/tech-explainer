...
The [multigres-operator](https://github.com/multigres/multigres-operator) -- an operator for sharded PostgreSQL clusters -- used kubebuilder once for the initial project setup. It has 13 CRDs. kubebuilder knew about 4 of them before the PROJECT file was deleted. The controllers live in a domain-driven package structure that kubebuilder can't scaffold. The webhooks use a custom registration pattern. Every scaffold marker was removed from the source. What survived: the Makefile wiring and the `config/` directory layout.

This isn't unusual. A [survey of six major operators](kubebuilder/00-overview.md) (Vitess, Zalando Postgres, Argo CD, Prometheus Operator, cert-manager, CloudNative-PG) found that five out of six don't use kubebuilder's scaffolding at all. All six use controller-gen. The scaffolding CLI is the part most operators leave behind.
